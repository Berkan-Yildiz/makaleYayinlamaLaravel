                ...
                <div class="row">
                    <div class="col-3 my-2">
                        <select class="js-states form-control" tabindex="-1" id="theme_type" name="theme_type" style="display: none; width: 100%">
                            ...
                            <option value="1" {{ request()->get('theme_type') == '1' ? 'selected' : '' }}>Kendim içerik oluşturmak istiyorum</option>
                    ...
                    <div class="col-3 my-2">
                        <select class="js-states form-control" tabindex="-1" id="process" name="process" style="display: none; width: 100%">
                            <option value="{{ null }}">İşlem Türü Seçin</option>
                            ...
                            <option value="3" {{ request()->get('process') == '3' ? 'selected' : '' }}>Parola Sıfırlama İşlemi Tamamlandığında Maili İçeriği</option>
                        </select>
                    </div>
                    <div class="col-3 my-2">
                        <input type="text" class="form-control" placeholder="Mail içeriği" name="search_text" value="{{ request()->get("search_text") }}">
                    </div>
                    <hr>
                    <div class="col-6 mb-2 mx-auto d-flex">
            ...
            >
                ...
                <x-slot:rows>
                    @foreach($list as $email)
                        <tr id="row-{{ $email->id }}">
                            <td>{{ $email->name }}</td>
                            <td>{{ $email->themeType }}</td>
                            <td>{{ $email->process }}</td>
                            <td>
                                <a href="javascript:void(0)"
                                   class="btn btn-primary btn-sm btnModelThemeDetail"
                                   data-bs-toggle="modal"
                                   data-bs-target="#contentViewModal"
                                   data-id="{{ $email->id }}"
                                   data-content="{{ $email->body }}"
                                   data-theme-type="{{ $email->getRawOriginal('themeType') }}"
                                >
                            ...
                            <td>{{ \Carbon\Carbon::parse($email->created_at)->translatedFormat("d.m.Y") }}</td>
                            <td>
                                <div class="d-flex">
    ...
    <script>
        $(document).ready(function ()
        {
            ...
            $('.btnModelThemeDetail').click(function () {
                ...
                let themeType = $(this).data('themeType');
                if (themeType == 1)
                {
                    $('#jsonData').html(content.replaceAll('"',''))
                }else{
