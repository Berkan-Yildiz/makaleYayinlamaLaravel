            ...
            <div class="example-container">
                <div class="example-content">
                    @if($errors->any())
                        ...
                        @endforeach
                    @endif
                    <form action="{{ isset($article) ? route('articles.edit', ['id' => $article->id]) : route('articles.create') }}"
                          method="POST"
        ...
        $(document).ready(function (){
            ...
            $('#btnSave').click(function () {
                if (title.val().trim() === "" || title.val().trim() == null){
                    Swal.fire({
                        title:'Uyarı',
                        text:'Lütfen Makale Başlığı alanını doldurunuz',
                        confirmButtonText:'Tamam',
                        icon:'info'
                    });
                }else if(tags.val().trim().length < 3 || tags.val().trim() == null){
                    Swal.fire({
                        title:'Uyarı',
